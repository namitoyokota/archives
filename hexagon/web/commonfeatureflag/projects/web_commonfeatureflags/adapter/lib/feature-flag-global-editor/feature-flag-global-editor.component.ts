...
@Component({
    selector: 'hxgn-commonfeatureflags-global-editor',
    template: ``,
    styles: [
        `:host {
            display:flex;
...
export class FeatureFlagGlobalEditorComponent implements OnInit, AfterViewInit, OnDestroy {
    ...
    private async injectComponentAsync(): Promise<void> {
        this.ref = await this.layoutCompilerSrv.delegateInjectComponentPortalAsync(
            InjectableComponentNames.featureFlagGlobalEditor,
